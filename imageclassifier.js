/* https://github.com/SashiDo/teachablemachine-node/blob/master/index.js */
...
class SashiDoTeachableMachine {
  ...
  async classify(params) {
    ...
    if ((!imageUrl.startsWith('data:image/')) && (!isImage(imageUrl))) {
  ...
  async inference({ imageUrl }) {
    try {
      ...
      const correctContentType = (/(gif|jpe?g|tiff?|png|webp|bmp)$/i).test(contentType)
      if(!correctContentType){
        contentType = imageUrl.split('.').pop();        
      }
